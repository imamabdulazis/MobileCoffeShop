...
class GojekPayScreen extends StatefulWidget {
  ...
  const GojekPayScreen({Key key, this.uuid, this.total, this.number})
      : super(key: key);
...
class _GojekPayScreenState extends State<GojekPayScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: BlocProvider<GopayPaymentBloc>(
  ...
  _onPay(String uuid, int total) async {
    var dynamicLink = await dynamicLinkService.createDynamicLinkHome();
    // print('ini deeplinkdari button: $dynamicLink');
  }

  _buildRoundButton(BuildContext context, String uuid, int total) {
